...
def save_generated_distribution_and_perplexity(
    ...
    if scores is None or generated_ids is None:
        print(f"Warning: no scores for {save_path}.")
        return None
...
def process_directory(
    ...
    for img_num, image_path in tqdm(
        ...
        if process_image(image_path, model, processor, img_num, output_dir):
            successful += 1

    print(f"Successfully processed {successful}/{len(image_paths)} images for {split}/{condition}")
    return successful
